...
#### Move-Based Chains (Sui, Aptos, Movement)
...
| CLI | Sui CLI 1.x | Sui CLI < 1.0 |
| SDK (TypeScript) | @mysten/sui 1.x | @mysten/sui < 0.50 |
| SDK (Rust) | sui-sdk | - |
...
| SDK (TypeScript) | @aptos-labs/ts-sdk 1.x | aptos (deprecated) |
| SDK (Rust) | aptos-sdk | - |
...
| SDK (TypeScript) | @solana/web3.js 2.x | @solana/web3.js 1.x |
...
| Category | Preferred | Avoid |
|----------|-----------|-------|
| Smart Contract Language | Rust (near-sdk 5.x), TypeScript | AssemblyScript |
...
| SDK | @polkadot/api 12.x | @polkadot/api < 10.0 |
...
1. **Never hardcode RPC URLs** - always use configuration files
2. **Use environment variables** for sensitive data (API keys, private RPCs)
3. **Implement fallback RPCs** for production applications
4. **Rate limit awareness** - public RPCs have limits, use dedicated providers for production
5. **Network detection** - always verify the connected network matches expected chainId
6. **Explorer links** - construct transaction/address links dynamically based on network
...
| 4 | Read `AGENT.md` | Understand coding standards and conventions |
